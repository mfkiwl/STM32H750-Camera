...
void LCD_puts_image_RGB565_DMA(const uint16_t * image)
{
  ...
  HAL_DMA_Start_IT(&hdma_memtomem_dma1_stream0, (uint32_t)image, (uint32_t)0xC0080000, 480 * 136);  // 320 * 240);
}

void LCD_puts_image_RGB565_DMA_REMAINDER(const uint32_t image, uint32_t len)
{
	HAL_DMA_Start_IT(&hdma_memtomem_dma1_stream0, image, (uint32_t)0xC0080000, len);
}
